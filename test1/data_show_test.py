...
def draw_by_one_data(i,pic_path,data1,data2):
    index_x = int(i % 13)
    index_y = int(i / 13)
    sub_x = data1[0] * 32
    sub_y = data1[1] * 32
    center_x = sub_x + 32 * index_x
    center_y = sub_y + 32 * index_y
    width = data1[2] * 416
    height = data1[3] * 416
...
def cac_one_pic_and_save():
    root = r"D:\code\PythonCode\tobject_detect\test1\imgs\imgs"
    ...
    for pic_path in pic_paths:
        ...
        results = torch.split(result,[5,6],1)
        for i in range(results[0].size()[0]):
            data1 = results[0][i]
            data2 = results[1][i]
            if (data1[4]<0.6):
                continue
            draw_by_one_data(i,pic_path,data1,data2)
        pass
