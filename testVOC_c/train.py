...
def train():
    ...
    for c in range(1000):
        ...
        for epoch in range(EPOCH):
            ...
            for ii,(data,label1) in enumerate(train_dataloader):
                input = Variable(data).cuda()
                target1 = Variable(label1)
                ...
                loss = loss_func1(score,target1.cuda())


                # loss = loss3
                if ii%100==0 and ii!=0:
                    print("epoch(%d) step(%d) loss(%0.6f) percent(%0.6f)"%(
                        epoch,
                        ii,
                        loss_meter.value()[0],
            ...
            # else:
            logger.info("while(%d) epoch(%d) train_loss(%0.6f)"%
                  (c,epoch,loss_meter.value()[0]))
